...
export default function UserDetailPage() {
  ...
  return (
    <div className="space-y-space-6 font-mono text-sm">
      ...
      <div className="flex flex-col gap-space-4 border-b border-border-default pb-space-5">
        <Link href="/users" className="inline-flex items-center gap-space-1.5 text-xs text-text-secondary hover:text-text-primary transition-fast w-fit focus-visible:ring-2 focus-visible:ring-accent-primary">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-space-4">
          <div className="flex items-center gap-space-3.5">
            <div className="flex h-12 w-12 items-center justify-center rounded-md bg-surface-raised border border-border-default text-md font-bold text-text-primary">
              {(user.name || 'P').slice(0, 2).toUpperCase()}
            </div>
            <div>
              <div className="flex items-center gap-space-2">
                <h1 className="text-xl font-semibold tracking-tight text-text-primary">{user.name || 'Player'}</h1>
                {user.is_blocked ? (
              ...
              <div className="flex items-center gap-space-2 text-xs text-text-tertiary mt-0.5">
                <span>ID: {user.id}</span>
                <button 
                  type="button"
                  onClick={() => copyToClipboard(user.id)} 
                  className="hover:text-text-primary p-0.5 rounded-xs transition-fast focus-visible:ring-2 focus-visible:ring-accent-primary" 
        ...
        <div 
          ...
          className="flex items-center overflow-x-auto border-t border-border-muted pt-space-2 scrollbar-none gap-space-1"
        >
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`flex items-center gap-space-1.5 whitespace-nowrap rounded-xs px-space-3 py-space-1.5 text-xs font-medium transition-fast focus-visible:ring-2 focus-visible:ring-accent-primary ${
                  isActive
                    ? 'bg-border-default text-text-primary shadow-sm'
                    : 'text-text-secondary hover:bg-surface-raised hover:text-text-primary'
                }`}
              >
                <Icon className={`h-3.5 w-3.5 ${isActive ? 'text-accent-primary' : 'text-text-tertiary'}`} aria-hidden="true" />
                <span>{tab.label}</span>
                {typeof tab.count === 'number' && tab.count > 0 && (
                  <span className="rounded-full bg-surface-muted px-space-1.5 py-0.5 text-[10px] text-text-secondary">
                    {tab.count}
                  </span>
                )}
              </button>
